...
int main(int argc, char *argv[])
{
    ...
    while (running) {
	 ...
	 if (counter == 50000) {
        ...
        /* Estrutura para pegar o tempo gasto an escrita*/
        ...
        //calculando a diferença de tempo para pegar o tempo gasto
        t_elapsed = t_stop.tv_usec - t_start.tv_usec;
        //printf("start: %ld and end: %ld\n", t_start.tv_usec, t_stop.tv_usec);

        //Estrutura para pegar o tamanho do arquivo escrito
        FILE *p_file = fopen("OUT.bmp","rb");
        fseek(p_file,0,SEEK_END);
        int size = ftell(p_file);
        fclose(p_file);

        printf("Size: %d bytes and time: %ld microseconds\n", size, t_elapsed);
        float perf = (float) size / (float) t_elapsed * 1000000.0 / 1024.0 / 1024.0; //convertendo para segundos e depois para MB
        printf("Performance de disco: %0.3f MB/s\n", perf);
        return 0;
        }
    }
}
