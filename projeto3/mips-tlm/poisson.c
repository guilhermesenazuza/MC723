...
void rhs ( int nx, int ny);
void sweep ( int nx, int ny, double dx, double dy,
 int itold, int itnew, int procNumber );
void timestamp ( void );
...
*/
    sweep ( nx, ny, dx, dy, itold, itnew, procNumber );
/*
...
*/
    ...
    if (procNumber == CORES-1)
      ...
      limitx = (procNumber+1)*nx/CORES;

    for (j = 0; j < ny; j++) {
        for (i = procNumber*nx/CORES; i < limitx; i++){
            u[i][j] = unew[i][j];
        }
    }
/*
